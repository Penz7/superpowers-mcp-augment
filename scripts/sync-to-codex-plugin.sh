...
#
# Sync this superpowers-mcp-augment checkout → prime-radiant-inc/openai-codex-plugins.
...
# plugins/superpowers-mcp-augment/ when absent, then copies the tracked plugin files from
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/superpowers-mcp-augment-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/superpowers-mcp-augment-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/superpowers-mcp-augment/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap superpowers-mcp-augment v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the superpowers-mcp-augment plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync superpowers-mcp-augment v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from superpowers-mcp-augment upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
