...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superpowers-mcp-augment/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/superpowers-mcp-augment/.fixture-keep"
    cat > "$repo/plugins/superpowers-mcp-augment/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/superpowers-mcp-augment/.fixture-keep
    git -C "$repo" add plugins/superpowers-mcp-augment/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/superpowers-mcp-augment/skills/example/agents"

    cat > "$repo/plugins/superpowers-mcp-augment/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/superpowers-mcp-augment/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superpowers-mcp-augment/.codex-plugin" \
        "$repo/plugins/superpowers-mcp-augment/.private-journal" \
        "$repo/plugins/superpowers-mcp-augment/assets" \
        "$repo/plugins/superpowers-mcp-augment/skills/example/agents" \
        "$repo/plugins/superpowers-mcp-augment/skills/example"

    cat > "$repo/plugins/superpowers-mcp-augment/.codex-plugin/plugin.json" <<EOF
{
  "name": "superpowers-mcp-augment",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/superpowers-mcp-augment/assets/superpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/superpowers-mcp-augment/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/superpowers-mcp-augment/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/superpowers-mcp-augment/.codex-plugin/plugin.json \
        plugins/superpowers-mcp-augment/assets/app-icon.png \
        plugins/superpowers-mcp-augment/assets/superpowers-small.svg \
        plugins/superpowers-mcp-augment/skills/example/agents/openai.yaml \
        plugins/superpowers-mcp-augment/skills/example/SKILL.md \
        plugins/superpowers-mcp-augment/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superpowers-mcp-augment/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superpowers-mcp-augment/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/superpowers-mcp-augment/.private-journal/leak.txt"
    git -C "$repo" add plugins/superpowers-mcp-augment/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/superpowers-mcp-augment/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/superpowers-mcp-augment/skills/example/agents/openai.yaml"
    ...
    assert_branch_absent "$dest" "sync/superpowers-mcp-augment-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/superpowers-mcp-augment/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/superpowers-mcp-augment" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/superpowers-mcp-augment-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/superpowers-mcp-augment'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/superpowers-mcp-augment-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/superpowers-mcp-augment-*" "Clean no-op local apply does not create sync branch in destination checkout"
